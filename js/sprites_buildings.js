...
const BUILDING_PAINTERS = {
  ...
  // M12 wave four. Fallback painters only -- js/sprites.js reaches these when a sheet has not loaded,
  // and the default without them is `BUILDING_PAINTERS.supply_depot`, i.e. a Terran box where a Zerg
  // warren should be. Each keeps the silhouette its model carries so the two do not disagree.
  roach_warren(h, c, W, H, TC) { BP.zBase(h, c, W, H, TC); h.E(W / 2 + 2, H / 2 + 2, 20, 14, '#5a3a4a', OUT, 1.5); h.E(W / 2 + 2, H / 2 + 2, 13, 9, '#1e1018', null); for (let k = 0; k < 5; k++) { const a = k * 1.256 + .4; h.L(W / 2 + 2 + Math.cos(a) * 16, H / 2 + 2 + Math.sin(a) * 11, W / 2 + 2 + Math.cos(a) * 26, H / 2 + 2 + Math.sin(a) * 18, TCOL.bone, 2.5); } },
  infestation_pit(h, c, W, H, TC) { BP.zBase(h, c, W, H, TC); h.E(W / 2, H / 2, 26, 17, '#1c2412', OUT, 1.5); for (let k = 0; k < 3; k++) { const a = k * 2.094 + .5; h.C(W / 2 + Math.cos(a) * 12, H / 2 + Math.sin(a) * 8, 8, '#5d7a3c', OUT, 1); h.C(W / 2 + Math.cos(a) * 12 - 2, H / 2 + Math.sin(a) * 8 - 2, 3, '#a8f060', null); } },
  // One tile, so it is three shapes and no more -- anything else is mush at 32 px.
  creep_tumour(h, c, W, H, TC) { h.E(W / 2, H / 2, W * .45, H * .38, '#4a2f3c', OUT, 1.5); h.E(W / 2, H / 2 - 1, W * .26, H * .2, '#b46aa8', null); h.E(W / 2, H / 2 - 1, W * .12, H * .1, TC, null); },
  nydus_worm(h, c, W, H, TC) { h.E(W / 2, H * .72, W * .46, H * .2, '#3a2530', OUT, 1.5); h.E(W / 2, H / 2, W * .34, H * .36, '#6f4f64', OUT, 1.5); h.E(W / 2, H * .44, W * .2, H * .18, '#1e1018', null); for (let k = 0; k < 6; k++) { const a = k * 1.047 + .3; h.L(W / 2 + Math.cos(a) * W * .18, H * .44 + Math.sin(a) * H * .16, W / 2 + Math.cos(a) * W * .3, H * .44 + Math.sin(a) * H * .3, TCOL.bone, 2); } h.E(W / 2, H * .74, 7, 3, TC, null); },
...
const BUILDING_ANIM = {
  ...
  missile_turret(ctx, u, x0, y0, W, H, f) { const cx = x0 + W / 2, cy = y0 + H / 2; const a = u.facing; ctx.fillStyle = '#5a636c'; ctx.strokeStyle = OUT; ctx.lineWidth = 1; ctx.save(); ctx.translate(cx, cy); ctx.rotate(a); ctx.beginPath(); ctx.roundRect(-6, -7, 22, 14, 2); ctx.fill(); ctx.stroke(); ctx.fillStyle = '#c33'; ctx.fillRect(10, -5, 4, 3); ctx.fillRect(10, 2, 4, 3); ctx.restore(); },
