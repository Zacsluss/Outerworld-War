...
Object.assign(UI, {
  ...
  drawUnitInfo(ctx, u, x, y, w, h) {
    ...
    if (u.isBuilding) { const s = Sprites.building(u); const k = Math.min(72 / s.W, 64 / s.H) * 1.15; ctx.scale(k, k); ctx.drawImage(s.cv, -s.W / 2 - s.M, -s.H / 2 - s.M); }
    else { const s = Sprites.unit(u, Sprites.dirOf(-Math.PI / 2 + Math.sin(G.frame * 0.02) * 0.5)); const k = Math.min(2.4, 36 / u.r); ctx.scale(k, k); ctx.drawImage(s.cv, -s.ox, -s.oy); }
