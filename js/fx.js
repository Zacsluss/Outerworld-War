...
const FX = {
  ...
  drawEffect(ctx, e) {
    ...
    switch (e.kind) {
      ...
      case 'rocket': { const [x, y] = this.lerpPos(e, 9); const a = Math.atan2(e.ty - e.y, e.tx - e.x); if (e.t > 1) this.p({ x, y, vx: 0, vy: -6, life: 0.5, r: 3, c: 'rgba(150,140,130,0.5)', g: -0.02 }); ctx.translate(x, y); ctx.rotate(a); ctx.fillStyle = e.color; ctx.fillRect(-5, -1.6, 10, 3.2); ctx.beginPath(); ctx.moveTo(5, -1.6); ctx.lineTo(8, 0); ctx.lineTo(5, 1.6); ctx.closePath(); ctx.fill(); ctx.globalCompositeOperation = 'lighter'; ctx.fillStyle = 'rgba(255,190,90,0.85)'; ctx.beginPath(); ctx.arc(-6, 0, 2.6, 0, 7); ctx.fill(); break; }
