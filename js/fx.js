...
const FX = {
  ...
  decal(o) { this.decals.push(Object.assign({ t: 0 }, o)); if (this.decals.length > 400) this.decals.shift(); },
  ...
  drawDecals(ctx, inView, visNow) {
    for (const d of this.decals) {
      ...
      else if (d.kind === 'corpse' || d.kind === 'wreck') { const def = DATA.all[d.def]; if (!def) continue; const fake = { def, owner: d.owner, r: def.r || 10, sieged: false }; const s = Sprites.unit(fake, Sprites.dirOf(d.facing)); ctx.save(); ctx.globalAlpha = a * 0.85; ctx.filter = d.kind === 'corpse' ? 'brightness(0.55) sepia(0.6) hue-rotate(-30deg)' : 'brightness(0.35) grayscale(0.7)'; ctx.translate(d.x, d.y); ctx.scale(1, 0.75); ctx.drawImage(s.cv, -s.ox, -s.oy); ctx.restore(); }
      ctx.globalAlpha = 1;
    }
  },
  ...
  drawEffect(ctx, e) {
    ...
    ctx.save();
    switch (e.kind) {
      case 'bullet': { ctx.globalCompositeOperation = 'lighter'; if (e.t >= 3) { const g = ctx.createRadialGradient(e.x, e.y, 0, e.x, e.y, 9); g.addColorStop(0, 'rgba(255,240,180,0.95)'); g.addColorStop(1, 'rgba(255,160,40,0)'); ctx.fillStyle = g; ctx.beginPath(); ctx.arc(e.x, e.y, 9, 0, 7); ctx.fill(); } ctx.strokeStyle = 'rgba(255,230,150,0.55)'; ctx.lineWidth = 1.2; ctx.beginPath(); ctx.moveTo(e.x, e.y); ctx.lineTo(e.tx, e.ty); ctx.stroke(); break; }
