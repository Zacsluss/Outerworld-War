...
const FX = {
  ...
  decal(o) { this.decals.push(Object.assign({ t: 0, born: G.frame }, o)); if (this.decals.length > this.MAX_DECALS) this.decals.shift(); },
  // Once per SIM frame per effect. drawEffect runs on every drawn frame and the simulation is drawn two
  // or three times per tick at 60 Hz, so a spawn gated on `e.t` alone fired two or three times -- the
  // same trap ambient() documents, in five more places. (REVIEW-M17)
  spawned: new WeakMap(),
  once(e) { if (this.spawned.get(e) === G.frame) return false; this.spawned.set(e, G.frame); return true; },
  ...
  spawn(e) {
    switch (e.kind) {
      ...
      // FIXLIST-M15 A1. Impact particles for the new kinds, so a shot lands rather than just stopping.
      case 'gauss': this.sparks(e.tx, e.ty, 2, 100); break;
      case 'battery': this.sparks(e.tx, e.ty, 4, 130); break;
      case 'shell': this.sparks(e.tx, e.ty, 5, 150); this.smoke(e.x, e.y, 2, 0.8); break;
      case 'frag': this.sparks(e.tx, e.ty, 4, 120); break;
      case 'rocket': this.smoke(e.x, e.y, 2, 0.7); break;
      case 'flak': this.smoke(e.tx, e.ty, 3, 0.9); break;
      case 'claw': case 'punch': this.sparks(e.tx, e.ty, 2, 60); break;
      case 'needle': this.sparks(e.tx, e.ty, 2, 70); break;
      case 'blade': this.sparks(e.tx, e.ty, 4, 90); break;
      case 'psi': this.sparks(e.tx, e.ty, 5, 110); break;
  ...
  update(dt) {
    ...
    if (G.frame % 24 === 0 && this._decF !== G.frame) { /* per SIM frame, not per drawn frame: decals faded 2.5x too fast at 60 Hz (REVIEW-M17) */ this._decF = G.frame; const ds = this.decals; for (let i = ds.length - 1; i >= 0; i--) { ds[i].t += 24; if (ds[i].t > ds[i].max) ds.splice(i, 1); } if (ds.length > this.MAX_DECALS) ds.splice(0, ds.length - this.MAX_DECALS); }
  },
  drawParticles(ctx) {
  ...
  drawEffect(ctx, e) {
    ...
    switch (e.kind) {
      ...
      // ---- FIXLIST-M15 A1: thirteen new shot kinds. Before this, 34 of the 63 armed units and
      // buildings in the game fired the same small white dot, because Combat.visual picked the look
      // from a hardcoded list of unit ids and everything absent fell through to 'bullet'.
      //
      // The three races are disjoint in KIND and not merely in colour, so a zoomed-out fight stays
      // readable: Terran fires solid objects (gauss/frag/shell/rocket/flak), Zerg fires organic
      // matter (claw/needle/acid), Protoss fires energy (plasma/beam/psi/blade).
      //
      // Each duration below MUST match Combat.SHOT_T for that kind, because lerpPos divides by it.
      // A mismatch does not throw, it just makes the shot arrive early or late -- which is why
      // test/shots.js checks the two tables agree rather than trusting this comment.
      case 'gauss': { ctx.globalCompositeOperation = 'lighter'; const a = Math.atan2(e.ty - e.y, e.tx - e.x); const mx = e.x + Math.cos(a) * 11, my = e.y + Math.sin(a) * 11; if (e.t >= 3) { ctx.fillStyle = e.color; ctx.globalAlpha = 0.9; ctx.beginPath(); ctx.arc(mx, my, 3.5, 0, 7); ctx.fill(); } ctx.globalAlpha = 0.7 * (e.t / 4); ctx.strokeStyle = e.color; ctx.lineWidth = 1; ctx.beginPath(); ctx.moveTo(mx, my); ctx.lineTo(e.tx, e.ty); ctx.stroke(); break; }
      case 'frag': { const [x, y] = this.lerpPos(e, 5); ctx.translate(x, y); ctx.rotate(G.frame * 0.9); ctx.fillStyle = e.color; ctx.fillRect(-2.5, -1.5, 5, 3); ctx.fillStyle = 'rgba(60,40,20,0.8)'; ctx.fillRect(-2.5, -1.5, 2, 3); break; }
      case 'shell': { ctx.globalCompositeOperation = 'lighter'; const a = Math.atan2(e.ty - e.y, e.tx - e.x); const mx = e.x + Math.cos(a) * 16, my = e.y + Math.sin(a) * 16; if (e.t >= 5) { const g = ctx.createRadialGradient(mx, my, 0, mx, my, 15); g.addColorStop(0, 'rgba(255,235,180,0.95)'); g.addColorStop(1, 'rgba(255,120,20,0)'); ctx.fillStyle = g; ctx.beginPath(); ctx.arc(mx, my, 15, 0, 7); ctx.fill(); } const [x, y] = this.lerpPos(e, 7); ctx.globalAlpha = 0.95; ctx.strokeStyle = e.color; ctx.lineWidth = 3.5; ctx.lineCap = 'round'; ctx.beginPath(); ctx.moveTo(x - Math.cos(a) * 9, y - Math.sin(a) * 9); ctx.lineTo(x, y); ctx.stroke(); break; }
      case 'rocket': { const [x, y] = this.lerpPos(e, 9); const a = Math.atan2(e.ty - e.y, e.tx - e.x); if (e.t > 1 && this.once(e)) this.p({ x, y, vx: 0, vy: -6, life: 0.5, max: 0.5, size: 3, col: [150, 140, 130], add: false, kind: 'smoke', shrink: false, grav: -0.02 });   /* p() reads size/col/grav; the old r/c/g keys drew an additive orange dot */ ctx.translate(x, y); ctx.rotate(a); ctx.fillStyle = e.color; ctx.fillRect(-5, -1.6, 10, 3.2); ctx.beginPath(); ctx.moveTo(5, -1.6); ctx.lineTo(8, 0); ctx.lineTo(5, 1.6); ctx.closePath(); ctx.fill(); ctx.globalCompositeOperation = 'lighter'; ctx.fillStyle = 'rgba(255,190,90,0.85)'; ctx.beginPath(); ctx.arc(-6, 0, 2.6, 0, 7); ctx.fill(); break; }
      case 'flak': { const T = 10, k = 1 - e.t / T; if (k < 0.62) { const [x, y] = this.lerpPos(e, T); ctx.fillStyle = e.color; ctx.beginPath(); ctx.arc(x, y, 2.2, 0, 7); ctx.fill(); } else { const b = (k - 0.62) / 0.38; ctx.globalAlpha = 1 - b; ctx.fillStyle = 'rgba(70,60,55,0.9)'; for (let i = 0; i < 6; i++) { const an = i * 1.047 + e.tx * 0.01, rr = 3 + b * 11; ctx.beginPath(); ctx.arc(e.tx + Math.cos(an) * rr, e.ty + Math.sin(an) * rr, 3.2 - b * 1.4, 0, 7); ctx.fill(); } ctx.globalCompositeOperation = 'lighter'; ctx.globalAlpha = (1 - b) * 0.8; ctx.fillStyle = e.color; ctx.beginPath(); ctx.arc(e.tx, e.ty, 4 + b * 5, 0, 7); ctx.fill(); } break; }
      case 'battery': { ctx.globalCompositeOperation = 'lighter'; const a = e.t / 7; const an = Math.atan2(e.ty - e.y, e.tx - e.x); const sx = e.x + Math.cos(an) * 14, sy = e.y + Math.sin(an) * 14; ctx.globalAlpha = a * 0.7; ctx.strokeStyle = e.color; ctx.lineWidth = 5; ctx.lineCap = 'butt'; ctx.beginPath(); ctx.moveTo(sx, sy); ctx.lineTo(e.tx, e.ty); ctx.stroke(); ctx.globalAlpha = a; ctx.lineWidth = 1.6; ctx.strokeStyle = 'rgba(255,255,230,0.95)'; ctx.beginPath(); ctx.moveTo(sx, sy); ctx.lineTo(e.tx, e.ty); ctx.stroke(); break; }
      case 'plasma': { const [x, y] = this.lerpPos(e, 7); ctx.globalCompositeOperation = 'lighter'; const g = ctx.createRadialGradient(x, y, 0, x, y, 9); g.addColorStop(0, '#fff'); g.addColorStop(0.35, e.color); g.addColorStop(1, 'rgba(40,80,255,0)'); ctx.fillStyle = g; ctx.beginPath(); ctx.arc(x, y, 9, 0, 7); ctx.fill(); break; }
      case 'beam': { ctx.globalCompositeOperation = 'lighter'; const a = e.t / 10; ctx.globalAlpha = a * 0.55; ctx.strokeStyle = e.color; ctx.lineWidth = 9; ctx.lineCap = 'round'; ctx.beginPath(); ctx.moveTo(e.x, e.y); ctx.lineTo(e.tx, e.ty); ctx.stroke(); ctx.globalAlpha = a; ctx.strokeStyle = '#fff'; ctx.lineWidth = 2.5; ctx.beginPath(); ctx.moveTo(e.x, e.y); ctx.lineTo(e.tx, e.ty); ctx.stroke(); ctx.globalAlpha = a * 0.8; ctx.fillStyle = e.color; ctx.beginPath(); ctx.arc(e.tx, e.ty, 5, 0, 7); ctx.fill(); break; }
      case 'psi': { ctx.globalCompositeOperation = 'lighter'; ctx.globalAlpha = e.t / 8; ctx.strokeStyle = e.color; ctx.lineWidth = 2.2; ctx.lineJoin = 'round'; const dx = e.tx - e.x, dy = e.ty - e.y, n = 6; ctx.beginPath(); ctx.moveTo(e.x, e.y); for (let i = 1; i <= n; i++) { const f = i / n, px = -dy, py = dx, l = Math.hypot(px, py) || 1, j = i === n ? 0 : ((i * 2654435761 + e.t * 40503) % 100 / 100 - 0.5) * 16; ctx.lineTo(e.x + dx * f + px / l * j, e.y + dy * f + py / l * j); } ctx.stroke(); break; }
      case 'blade': { ctx.globalCompositeOperation = 'lighter'; ctx.globalAlpha = e.t / 5; const a = Math.atan2(e.ty - e.y, e.tx - e.x); ctx.translate(e.tx, e.ty); ctx.rotate(a); ctx.strokeStyle = e.color; ctx.lineWidth = 3; ctx.lineCap = 'round'; ctx.beginPath(); ctx.arc(0, 0, 11, -0.9, 0.9); ctx.stroke(); ctx.globalAlpha = e.t / 10; ctx.lineWidth = 1.2; ctx.beginPath(); ctx.arc(0, 0, 15, -0.7, 0.7); ctx.stroke(); break; }
      case 'claw': { ctx.globalAlpha = e.t / 5; ctx.strokeStyle = e.color; ctx.lineWidth = 1.8; ctx.lineCap = 'round'; const a = Math.atan2(e.ty - e.y, e.tx - e.x); ctx.translate(e.tx, e.ty); ctx.rotate(a); for (let i = -1; i <= 1; i++) { ctx.beginPath(); ctx.moveTo(-7, i * 4.5 - 2); ctx.lineTo(5, i * 4.5 + 2); ctx.stroke(); } break; }
      case 'punch': { ctx.globalAlpha = e.t / 5; ctx.strokeStyle = e.color; ctx.lineWidth = 2; ctx.beginPath(); ctx.arc(e.tx, e.ty, 8 - e.t, 0, 7); ctx.stroke(); break; }
      case 'needle': { const [x, y] = this.lerpPos(e, 5); const a = Math.atan2(e.ty - e.y, e.tx - e.x); ctx.translate(x, y); ctx.rotate(a); ctx.strokeStyle = e.color; ctx.lineWidth = 1.6; ctx.lineCap = 'round'; ctx.beginPath(); ctx.moveTo(-6, 0); ctx.lineTo(6, 0); ctx.stroke(); ctx.lineWidth = 1; ctx.beginPath(); ctx.moveTo(-6, 0); ctx.lineTo(-9, -2.5); ctx.moveTo(-6, 0); ctx.lineTo(-9, 2.5); ctx.stroke(); break; }
      case 'acid': { const T = 7; const [x, y] = this.lerpPos(e, T); const k = 1 - e.t / T; ctx.globalAlpha = 0.9; ctx.fillStyle = e.color; const w = 4.5 + Math.sin(k * 9) * 1.1; ctx.beginPath(); ctx.ellipse(x, y, w, 9 - w * 0.6, Math.atan2(e.ty - e.y, e.tx - e.x), 0, 7); ctx.fill(); if (e.t <= 2) { ctx.globalAlpha = e.t / 2 * 0.7; for (let i = 0; i < 5; i++) { const an = i * 1.257; ctx.beginPath(); ctx.arc(e.tx + Math.cos(an) * 7, e.ty + Math.sin(an) * 7, 2, 0, 7); ctx.fill(); } } break; }
      ...
      case 'flame': { const a = Math.atan2(e.ty - e.y, e.tx - e.x); if (this.once(e)) for (let k = 0; k < 5; k++) { const sp = 120 + this.rnd() * 120, da = (this.rnd() - .5) * 0.7; this.p({ x: e.x + Math.cos(a) * 10, y: e.y + Math.sin(a) * 10, vx: Math.cos(a + da) * sp, vy: Math.sin(a + da) * sp, life: 0.25 + this.rnd() * 0.15, max: 0.4, size: 5 + this.rnd() * 4, col: this.rnd() < 0.5 ? [255, 190, 60] : [255, 90, 20], add: true }); } break; }
      ...
      // same flame. (FX.rnd is Math.random, render-side, and a random flame would have been legal here; it is simply not needed.)
      //
      ...
      case 'boom': case 'bigboom': { const T = e.kind === 'boom' ? 18 : 40; const k = 1 - e.t / T; if (e.kind === 'bigboom' && (e.t === 30 || e.t === 20 || e.t === 12) && this.once(e)) { const ox = (this.rnd() - .5) * e.r * 1.2, oy = (this.rnd() - .5) * e.r; this.fire(e.x + ox, e.y + oy, 14, 130); this.smoke(e.x + ox, e.y + oy, 6, 1.6); } if (k < 0.4) { ctx.globalCompositeOperation = 'lighter'; const rr = (e.r || 14) * (0.6 + k * 2.5); const g = ctx.createRadialGradient(e.x, e.y, 0, e.x, e.y, rr); g.addColorStop(0, `rgba(255,255,230,${(0.4 - k) * 2})`); g.addColorStop(0.4, `rgba(255,180,60,${(0.4 - k) * 1.5})`); g.addColorStop(1, 'rgba(255,60,0,0)'); ctx.fillStyle = g; ctx.beginPath(); ctx.arc(e.x, e.y, rr, 0, 7); ctx.fill(); } break; }
