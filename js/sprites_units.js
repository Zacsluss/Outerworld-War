...
const UNIT_PAINTERS = {
  ...
  // M12 wave four. These are the FALLBACK painters, not the art: js/sprites.js asks the atlas first and
  // only reaches here when a sheet has not loaded. They exist because the alternative is
  // `UNIT_PAINTERS[id] || UNIT_PAINTERS.marine` -- a roach drawn as a Terran marine, which is what M11
  // shipped for sixteen units and what a player eventually reported. Kept to the silhouette each
  // model's comment describes and nothing more.
  brood_cocoon(h, r, TC, TCd, st) { const { sw } = ANIM(st); h.E(0, 0, r * .75 * (1 + sw * .03), r * 1.0, '#8a6a58', OUT, 1.5); h.E(0, 0, r * .4, r * .6, 'rgba(150,90,60,0.6)', null); },
  roach(h, r, TC, TCd, st) { const { F, Fd, Cp, bone } = TCOL; const { sw, ak } = ANIM(st); h.legs(3, -r * .1, r * .6, r * 1.0, Fd, 2, sw); h.E(-r * .05, 0, r * .95, r * .75, '#6f5348'); h.E(r * .1, 0, r * .55, r * .5, Cp, null); h.E(-r * .3, 0, r * .32, r * .24, TC, OUT, 1); for (let k = 0; k < 3; k++) h.E(-r * .5 + k * r * .38, 0, r * .16, r * .55, '#4a3540', 'rgba(20,20,25,0.5)', 1); h.C(r * .7 + ak * r * .15, 0, r * .3, '#8ad24a', OUT, 1); h.C(r * .78 + ak * r * .2, 0, r * .12 + ak * r * .1, '#d6ff7a', null); },
  ravager(h, r, TC, TCd, st) { const { Fd, Cp, bone } = TCOL; const { sw, ak } = ANIM(st); h.legs(3, -r * .1, r * .7, r * 1.1, Fd, 2.5, sw); h.E(-r * .05, 0, r * 1.0, r * .8, '#63464a'); h.E(r * .15, 0, r * .5, r * .45, Cp, null); h.C(-r * .55, 0, r * .45, '#4f6a34'); h.C(-r * .55, 0, r * .2 + ak * r * .18, '#b8ff4a', null); h.E(-r * .1, 0, r * .3, r * .22, TC, OUT, 1); h.L(r * .5, -r * .45, r * 1.0, -r * .75, bone, 2.5); h.L(r * .5, r * .45, r * 1.0, r * .75, bone, 2.5); },
  baneling(h, r, TC, TCd, st) { const { Fd } = TCOL; const { sw, ak } = ANIM(st); h.legs(2, -r * .1, r * .45, r * .6, Fd, 1.5, sw); h.E(0, 0, r * .5, r * .45, '#6d6640'); h.C(r * .05, -r * .42, r * .38 * (1 + ak * .25), '#8fe03a'); h.C(r * .05, r * .42, r * .38 * (1 + ak * .25), '#8fe03a'); h.C(-r * .3, 0, r * .2, TC, OUT, 1); h.C(r * .5, 0, r * .18, '#4a3d30'); },
  swarm_host(h, r, TC, TCd, st) { const { Fd, bone } = TCOL; const { sw } = ANIM(st); h.legs(3, -r * .1, r * .5, r * 1.0, Fd, 2, sw); h.E(0, 0, r * .95, r * .85, '#5f5240'); h.C(-r * .15, 0, r * .45, '#20141c'); h.C(-r * .15, 0, r * .22, '#8ad24a', null); for (let k = 0; k < 4; k++) { const a = k * 1.571 + .4; h.L(-r * .15 + Math.cos(a) * r * .42, Math.sin(a) * r * .42, -r * .15 + Math.cos(a) * r * .8, Math.sin(a) * r * .8, bone, 2); } h.C(r * .65, 0, r * .24, '#6f4f64'); h.E(r * .35, 0, r * .2, r * .16, TC, OUT, 1); },
  locust(h, r, TC, TCd, st) { const { Fd, bone } = TCOL; const { sw, ak } = ANIM(st); h.legs(2, 0, r * .5, r * .7, Fd, 1.5, sw); h.E(-r * .05, 0, r * .7, r * .38, '#8a7a52'); h.E(-r * .25, 0, r * .22, r * .16, TC, OUT, 1); h.C(r * .55 + ak * r * .2, 0, r * .26, '#6b5c3a'); h.L(r * .7 + ak * r * .2, -r * .15, r * 1.05 + ak * r * .3, -r * .3, bone, 1.8); h.L(r * .7 + ak * r * .2, r * .15, r * 1.05 + ak * r * .3, r * .3, bone, 1.8); },
  viper(h, r, TC, TCd, st) { const { bone } = TCOL; const { sw, ak } = ANIM(st); h.wing(-1, r, r * 1.5, 'rgba(110,130,80,0.75)', OUT, sw); h.wing(1, r, r * 1.5, 'rgba(110,130,80,0.75)', OUT, -sw); h.E(0, 0, r * 1.0, r * .32, '#6d7a4e'); h.E(-r * .25, 0, r * .3, r * .2, TC, OUT, 1); h.C(r * .8, 0, r * .26, '#4e6030'); h.C(r * .92, 0, r * .09, '#c8f050', null); h.Q(r * .3, -r * .2, r * .9 + ak * r * .5, -r * .5, r * 1.3 + ak * r * .6, -r * .25, '#5b3d38', 2.5); h.Q(r * .3, r * .2, r * .9 + ak * r * .5, r * .5, r * 1.3 + ak * r * .6, r * .25, '#5b3d38', 2.5); h.Q(-r * .9, 0, -r * 1.4, sw * r * .2, -r * 1.7, sw * r * .35, '#5b3d38', 2.5); },
  infestor(h, r, TC, TCd, st) { const { Fd, bone } = TCOL; const { sw } = ANIM(st); h.legs(3, -r * .1, r * .6, r * 1.0, Fd, 2, sw); h.E(-r * .5, 0, r * .55, r * .48, '#7a9a58'); h.E(0, 0, r * .8, r * .48, '#566b3f'); h.E(-r * .2, 0, r * .28, r * .2, TC, OUT, 1); h.C(-r * .7, 0, r * .18, '#a8f060', null); h.C(r * .6, 0, r * .28, '#3f5230'); for (let k = 0; k < 3; k++) h.C(-r * .1 - k * r * .3, (k % 2 ? .32 : -.32) * r, r * .1, '#8fd050', null); },
  overseer(h, r, TC, TCd, st) { const { Cp, bone } = TCOL; const { sw } = ANIM(st); for (let k = 0; k < 5; k++) { const a = k * 1.256 + .5; h.L(-r * .2 + Math.cos(a) * r * .5, Math.sin(a) * r * .5, -r * .2 + Math.cos(a) * r * .95, Math.sin(a) * r * .95, bone, 2); } h.E(0, 0, r * 1.0, r * .82, '#7a6a72'); h.E(-r * .1, -r * .1, r * .7, r * .45, '#96828c', null); h.E(-r * .15, 0, r * .42, r * .3, TC, OUT, 1); h.C(r * .8, 0, r * .34, Cp); h.C(r * .92, 0, r * .22 * (1 + sw * .06), '#ffc04a', OUT, 1); h.C(r * 1.02, 0, r * .07, '#241c18', null); },
