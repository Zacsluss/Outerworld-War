...
const AI_SCRIPTS = {
  ...
  Z: [[11, 'spawning_pool'], [12, 'hatchery'], [13, 'extractor'], [18, 'hydralisk_den'], [20, 'creep_colony'], [22, 'lair'], [24, 'evolution_chamber'], [26, 'hatchery'], [28, 'extractor'], [34, 'spire'], [36, 'hatchery'], [44, 'queens_nest'], [48, 'extractor'], [52, 'hive'], [56, 'creep_colony'], [60, 'ultralisk_cavern'], [64, 'hatchery'], [66, 'defiler_mound'], [70, 'greater_spire'], [80, 'hatchery']],
...
const AI_RESEARCH = {
  ...
  Z: ['metabolic', 'muscular', 'grooved', 'lurker_aspect', 'carapace', 'missW', 'burrow_tech', 'flyW', 'pneumatized', 'meleeW', 'anabolic', 'chitinous', 'adrenal', 'consume_tech', 'plague_tech', 'flyA'],
...
class AI {
  constructor(p, diff) {
    this.p = p; this.diff = diff; this.step = 0; this.pending = {}; this.lastThink = 0; this.lastArmy = 0; this.state = 'gather'; this.target = null; this.attackN = 0; this.attackThreshold = diff === 'easy' ? 40 : diff === 'hard' ? 24 : 30; this.waves = 0;
  ...
  count(id, inclProd = true) { let n = 0; for (const u of G.units) { if (!u.alive || u.owner !== this.p.id) continue; if (u.def.id === id) n++; if (inclProd) for (const it of u.prod) if (it.id === id) n++; if (u.order.type === 'build' && u.order.def.id === id) n++; } if (this.pending[id] && G.frame - this.pending[id] < 600) n++; return n; }
  halls() { return this.mine(u => u.isBuilding && u.def.depot && !u.lifted); }
  enemies() { return G.players.filter(q => q.id !== this.p.id && !q.defeated); }
  tick() {
  ...
  economy() {
    ...
    gasB.forEach((g, gi) => {
      ...
      if (p.gas > 800 && p.minerals < 300) want = Math.min(want, 1); if (p.gas > 1500 && p.minerals < 400) want = 0;
    ...
    // worker production
    const want = Math.min(70, halls.length * 18 + gasB.length * 3);
  ...
  supply() {
    ...
    const prodN = this.mine(u => u.isBuilding && u.def.produces.length && u.done).length + (p.race === 'Z' ? this.halls().length * 2 : 0);
  ...
  macro() {
    ...
    if ((p.minerals > 450 && this.scriptIdx >= 6) || p.minerals > 700) {
      ...
      if (r === 'Z' && this.count('hatchery') + this.count('lair') + this.count('hive') < 8) this.build('hatchery');
  ...
  production() {
    ...
    if (this.race === 'Z' && p.hasTech('lurker_aspect') && (counts.lurker || 0) < (counts.hydralisk || 0) / 3 && p.minerals >= 50 && p.gas >= 100) { const h = this.mine(u => u.def.id === 'hydralisk' && u.order.type !== 'attack' && u.done); if (h.length) { Abilities.morph(h[0], 'lurker'); return; } }
  ...
  build(id) {
    ...
    const halls = this.halls(); const h = halls[Math.floor(Math.random() * Math.min(2, halls.length))] || { x: p.startX, y: p.startY };
  ...
  findSpot(def, cx, cy) {
    ...
    for (let r = 2; r < 24; r++) {
      for (let k = 0; k < 24; k++) {
        ...
        if (ok && def.psi) { let close = false; for (const u of G.units) if (u.alive && u.owner === p.id && u.def.psi && distPt(u.x, u.y, (tx + 1) * TILE, (ty + 1) * TILE) < 5 * TILE) close = true; if (close && Math.random() < 0.7) ok = false; }
        if (ok) return [tx, ty];
      }
    }
    return null;
  ...
  army() {
    ...
    for (const b of this.mine(u => u.isBuilding && (u.def.produces.length || u.def.spawnsLarva))) b.rally = { x: rally.x + (Math.random() - .5) * 64, y: rally.y + (Math.random() - .5) * 64 };
    // defense
    ...
    if (this.state === 'gather') {
      for (const u of army) if (u.order.type === 'idle' && distPt(u.x, u.y, rally.x, rally.y) > 5 * TILE) u.setOrder({ type: 'attackmove', x: rally.x + (Math.random() - .5) * 96, y: rally.y + (Math.random() - .5) * 96 });
    ...
    if (this.state === 'attack') {
      ...
      for (const u of rest) if (u.order.type === 'idle' && distPt(u.x, u.y, rally.x, rally.y) > 5 * TILE) u.setOrder({ type: 'attackmove', x: rally.x + (Math.random() - .5) * 96, y: rally.y + (Math.random() - .5) * 96 });
      ...
      for (const u of this.supportUnits()) { if (u.def.id === 'high_templar' || u.def.id === 'defiler') { if (u.order.type !== 'ability' && u.order.type !== 'follow') u.setOrder({ type: 'follow', target: army[0] || u }); } else if (u.order.type === 'idle' || u.order.type === 'move') u.setOrder({ type: 'follow', target: army[Math.floor(Math.random() * army.length)] || u }); }
  ...
  pickTarget(from) {
    ...
    for (const u of G.units) { if (!u.alive || u.owner === this.p.id || !u.isBuilding || u.def.tier === 'addon') continue; if (G.players[u.owner].defeated) continue; const d = distPt(u.x, u.y, from.x, from.y) - (u.def.depot ? 8 * TILE : 0); if (d < bd) { bd = d; best = u; } }
    if (!best) { for (const u of G.units) { if (u.alive && u.owner !== this.p.id && !G.players[u.owner].defeated && !u.def.larva) { const d = distPt(u.x, u.y, from.x, from.y); if (d < bd) { bd = d; best = u; } } } }
    return best;
  }
  micro() {
    const p = this.p;
    for (const u of G.units) {
      ...
      else if (d === 'vulture' && u.mines > 0 && p.hasTech('spider_mines_tech') && u.order.type === 'idle' && (G.frame + u.id) % 48 === 0) { Abilities.issue(u, 'spider_mine', null, u.x + (Math.random() - .5) * 64, u.y + (Math.random() - .5) * 64); }
