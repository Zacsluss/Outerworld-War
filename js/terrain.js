...
// ============================================================================
const Terrain = {
  CH: 8, chunks: new Map(), seed: 1, creepPat: null, mini: null,
  ...
  reset(seed) { this.seed = seed; this.chunks.clear(); this.creepPat = null; this.mini = null; },
  // palette
  lowCol(n, c) { // cracked dirt: n noise 0..1, c crack 0..1
    let r = 112 + n * 46, g = 90 + n * 38, b = 58 + n * 24; const crack = c > 0.9 ? (c - 0.9) * 6 : 0; r -= crack * 45; g -= crack * 38; b -= crack * 26; return [r, g, b];
  },
  highCol(n, c) { let r = 138 + n * 40, g = 122 + n * 34, b = 96 + n * 26; const plate = c > 0.93 ? (c - 0.93) * 8 : 0; r -= plate * 40; g -= plate * 36; b -= plate * 30; return [r, g, b]; },
  rampCol(n) { return [128 + n * 30, 114 + n * 26, 86 + n * 20]; },
  getChunk(cx, cy) {
  ...
  renderChunk(cx, cy) {
    ...
    for (let py = 0; py < px; py++) for (let pxx = 0; pxx < px; pxx++) {
      ...
      let col;
      if (cl === 2) { const k = 0.45 + n * 0.35; col = [70 * k + 20, 66 * k + 18, 60 * k + 16]; }
      else if (cl === 1) { const s = this.vnoise(wx / 6, wy / 20); const k = 0.5 + n * 0.3 + (s > 0.6 ? 0.15 : 0); col = [96 * k + 22, 80 * k + 18, 60 * k + 14]; }
      else {
    ...
    for (let ty = t0y - 1; ty < t0y + CH + 1; ty++) for (let tx = t0x - 1; tx < t0x + CH + 1; tx++) {
      ...
      if (cl === 1) {
        ...
        if (southLow) { // visible rock face
          const g = x.createLinearGradient(0, ly, 0, ly + TILE); g.addColorStop(0, 'rgba(150,120,90,0.55)'); g.addColorStop(0.35, 'rgba(90,66,48,0.35)'); g.addColorStop(1, 'rgba(30,20,14,0.75)'); x.fillStyle = g; x.fillRect(lx, ly, TILE, TILE);
          x.strokeStyle = 'rgba(20,12,8,0.5)'; x.lineWidth = 1; for (let k = 0; k < 4; k++) { const sx = lx + 4 + this.hash(tx * 7 + k, ty) * 24; x.beginPath(); x.moveTo(sx, ly + 6); x.lineTo(sx + (this.hash(tx, ty * 3 + k) - .5) * 8, ly + TILE); x.stroke(); }
          x.fillStyle = 'rgba(0,0,0,0.45)'; x.fillRect(lx, ly + TILE, TILE, 7); x.fillStyle = 'rgba(0,0,0,0.2)'; x.fillRect(lx, ly + TILE + 7, TILE, 5);
        } else { // rubble edge
          for (let k = 0; k < 3; k++) { const bx = lx + 6 + this.hash(tx * 3 + k, ty * 5) * 20, by = ly + 6 + this.hash(tx * 5, ty * 3 + k) * 20, br = 4 + this.hash(tx + k, ty - k) * 6; x.fillStyle = 'rgba(40,28,20,0.6)'; x.beginPath(); x.ellipse(bx + 1.5, by + 1.5, br, br * .7, 0, 0, 7); x.fill(); x.fillStyle = 'rgb(' + (110 + k * 12) + ',' + (92 + k * 10) + ',' + (70 + k * 8) + ')'; x.beginPath(); x.ellipse(bx, by, br, br * .7, 0, 0, 7); x.fill(); x.fillStyle = 'rgba(255,240,220,0.25)'; x.beginPath(); x.ellipse(bx - br * .3, by - br * .3, br * .4, br * .25, 0, 0, 7); x.fill(); }
        }
        if (northHigh) { x.fillStyle = 'rgba(255,240,210,0.28)'; x.fillRect(lx, ly, TILE, 3); }
      } else if (cl === 2) { // boulders
        const nb = 2 + Math.floor(this.hash(tx, ty) * 3);
        for (let k = 0; k < nb; k++) { const bx = lx + 4 + this.hash(tx * 11 + k, ty) * 24, by = ly + 4 + this.hash(tx, ty * 11 + k) * 24, br = 6 + this.hash(tx * 3 + k, ty * 7) * 9; x.fillStyle = 'rgba(0,0,0,0.45)'; x.beginPath(); x.ellipse(bx + 3, by + 3, br, br * .75, 0, 0, 7); x.fill(); const g = x.createRadialGradient(bx - br * .4, by - br * .4, 1, bx, by, br); g.addColorStop(0, '#8a8478'); g.addColorStop(1, '#3b352e'); x.fillStyle = g; x.beginPath(); x.ellipse(bx, by, br, br * .75, 0, 0, 7); x.fill(); }
        ...
        if (r < 0.025) { const cxp = lx + 16, cyp = ly + 16, cr = 8 + r * 200; x.strokeStyle = 'rgba(40,28,18,0.55)'; x.lineWidth = 3; x.beginPath(); x.ellipse(cxp, cyp, cr, cr * .7, 0, 0, 7); x.stroke(); x.fillStyle = 'rgba(60,44,30,0.35)'; x.beginPath(); x.ellipse(cxp, cyp, cr - 2, cr * .7 - 2, 0, 0, 7); x.fill(); x.strokeStyle = 'rgba(200,170,130,0.25)'; x.lineWidth = 1.5; x.beginPath(); x.ellipse(cxp, cyp - 1, cr, cr * .7, 0, Math.PI, Math.PI * 2); x.stroke(); }
        else if (r < 0.07) { x.strokeStyle = 'rgba(90,110,50,0.75)'; x.lineWidth = 1.5; const gx = lx + 8 + this.hash(tx, ty + 99) * 16, gy = ly + 10 + this.hash(tx + 99, ty) * 16; for (let k = 0; k < 6; k++) { x.beginPath(); x.moveTo(gx, gy + 6); x.lineTo(gx + (k - 2.5) * 2.4, gy - 4 - this.hash(k, tx + ty) * 6); x.stroke(); } }
        else if (r < 0.10) { for (let k = 0; k < 3; k++) { const bx = lx + 6 + this.hash(tx + k, ty * 2) * 20, by = ly + 6 + this.hash(tx * 2, ty + k) * 20, br = 2 + this.hash(tx * k + 1, ty) * 3; x.fillStyle = 'rgba(0,0,0,0.35)'; x.beginPath(); x.ellipse(bx + 1, by + 1, br, br * .7, 0, 0, 7); x.fill(); x.fillStyle = '#9a8a72'; x.beginPath(); x.ellipse(bx, by, br, br * .7, 0, 0, 7); x.fill(); } }
