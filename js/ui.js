...
const UI = {
  ...
  menuItems() {
    // The menu key by its bound name, as the HUD and the help say it: "(Esc)" written in stayed after the key was put
    // elsewhere (tenth session; item 6 made Escape the pause binding's default).
    const menuKey = this.keyName(this.key('pause')), keyed = s => s + (menuKey ? ' (' + menuKey + ')' : '');
    ...
    return { title: br ? 'PAUSED  (BRANCH)' : 'PAUSED',
      ...
      items: [[keyed('Resume'), () => { this.menu = null; }],
