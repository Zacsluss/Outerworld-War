...
const Z = {
  ...
  think() {
    ...
    if (canal && !this.done.nydus) { const hatch = Bot.mine(u => u.def.spawnsLarva && u.done).sort((a, b) => distPt(b.x, b.y, p.startX, p.startY) - distPt(a.x, a.y, p.startX, p.startY))[0]; const spot = hatch && Bot.findSpot(DATA.buildings.nydus_canal, Math.floor(hatch.x / TILE), Math.floor(hatch.y / TILE), 3); if (spot) { Bot.select([canal]); Bot.button('Build Nydus Exit'); Bot.lclick((spot[0] + 1) * TILE, (spot[1] + 1) * TILE); this.done.nydus = f; Bot.log('nydus exit ' + (canal.nydusLink ? 'placed' : 'FAILED')); if (!canal.nydusLink) Bot.issues.push('nydus exit could not be placed'); } }
