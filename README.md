...
- Balance is tuned by AI-vs-AI runs, not by human ladder play. **Terran vs Zerg is 79% [75-83] over 380 decided games, outside the 60/40 band the harness asks for.** It was re-measured from scratch after an M8 bug fix: Hold Position never checked the weapon cooldown, so any unit holding with a target in range fired 24 times a second. Fixing that moved TvZ *nine points toward Terran* -- the bug was net-helping Zerg, which spends more of a game defending -- and a second fix took eight back. PvT and PvZ have not been re-measured since and carry no trustworthy number. See HANDOFF.md.
