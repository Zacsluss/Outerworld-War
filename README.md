...
- **Multiplayer**: deterministic lockstep over a WebSocket relay built into serve.js. Everyone opens the LAN address and connects; the Multiplayer screen then lists the games hosted on that server. HOST GAME makes one (the relay picks its code), a click on a row joins one, and the lobby shows the teams, a ready mark per player, add-AI per team for the host, the map and speed, and a chat. A game that should not be listed is joined by a code instead (JOIN BY CODE): one relay holds many rooms and a code-joined room is never listed, so the code is what keeps strangers out of that one. In-game Enter is chat. Clients exchange a state hash every two seconds; a mismatch shows a desync banner and both sides download their command logs. If someone drops, their units stop on a frame the relay picks and the game continues; connecting again with the same name and room code rejoins the game from a snapshot a live player donates, with only the commands since replayed (or, with no donor, by re-simulating the relay's command history from the start). Room codes are at least four characters, an address may join sixty times a minute, and cheats are off in network games.
