...
A from-scratch browser remake of StarCraft: Brood War's gameplay. Plain HTML5 canvas + JavaScript, no build step, no dependencies, no Blizzard assets. All art is generated by the project itself: a noise-based Badlands-style tileset with cliff faces and doodads, unit and building sprite sheets baked from 3D models by the included pipeline (16 facings x 8 walk frames x 5 attack frames, oblique camera, lighting, team-colour masks, death collapse), additive particle effects with blood/scorch decals and corpses, and a bevelled BW-style console with icon buttons and a custom cursor. Audio is synthesized.
...
Units and buildings are 3D models (boxes, spheres, cylinders, cones, domes, crystals) described in [tools/models.js](tools/models.js) with reusable rigs (biped, bug, tank, walker, ship, flyer, Terran base, Zerg mound, Protoss slab) and joint animations. [tools/bake.js](tools/bake.js) renders them with a software rasterizer ([tools/raster.js](tools/raster.js): oblique 50-degree camera, Gouraud lighting, supersampling, outline pass) into sprite sheets: 16 facings x (1 idle + 8 walk + 5 attack) frames per unit, one image per building, plus a team-colour mask that the game multiplies with the player colour at load time.

```bash
node broodwar/tools/bake.js
```

That bakes everything in about 15 seconds and writes `assets/sprites/*.png`, `assets/atlas.js` and `assets/preview.png` (a contact sheet for review). Use `--only marine,hatchery` to rebake a few models. To change how something looks, edit its model in tools/models.js and rebake. If the assets are missing the game falls back to the vector painters in js/sprites_units.js and js/sprites_buildings.js.
...
- Art is baked from simple primitive 3D models rather than hand-sculpted ones; sound is synthesized beeps rather than voice lines.
