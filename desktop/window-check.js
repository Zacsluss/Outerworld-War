...
const hostAndGetPort = async s => { await s.dt.evaluate("document.getElementById('netName').value = 'Alice'; document.getElementById('netRoom').value = 'ABCD'; document.getElementById('netHost').click(); true"); return until(() => s.dt.quiet('Desktop.hosting ? Desktop.hosting.port : 0'), 15000); };
...
(async () => {
  ...
  ok(await dt.quiet("typeof G !== 'undefined' && typeof UI !== 'undefined' && typeof Net !== 'undefined' && !!document.getElementById('start')") === true, 'the game scripts loaded: G, UI, Net and the START button exist');
  ...
  ok(await until(() => dt.quiet("!!(Net.lobby && Net.lobby.room === 'ABCD' && Net.lobby.players.length === 1 && Net.lobby.players[0].name === 'Alice' && Net.lobby.players[0].host === true)"), 5000), 'the host is in the lobby of room ABCD on its own relay, as host', await dt.quiet('JSON.stringify(Net.lobby)'));
  ...
  ok(new RegExp(':' + port + '\\b').test(share) && /ABCD/.test(share) && /STOP HOSTING/.test(await dt.quiet("document.getElementById('netHost').textContent")), 'the share line names the port and the room, and the button reads STOP HOSTING', share);

  // a second player joins the sidecar from outside the app
  const B = client(port);
  try { await B.open; B.send({ t: 'join', name: 'Bob', race: 'Z', room: 'ABCD' }); await sleep(500); } catch (e) { ok(false, 'a second player could connect', String(e.message)); }
