...
- **`node test/all.js` is the gate before any commit.** 84 suites, ~4 minutes. Green means green. A red
  `rooms` with `EADDRINUSE` is another gate or a stray server on the relay ports (8793-8800; 8810-8813 for `lobby`, 8840 for `spectate`), not a fault:
